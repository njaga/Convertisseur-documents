...
const tools: ToolItem[] = [
  { to: '/modifier-pdf', title: 'Modifier PDF', description: 'Ajoutez du texte, des images, signatures, annotations et dessins directement sur vos pages.', category: 'PDF', icon: PencilLine, accent: 'indigo', featured: true },
  ...
  { to: '/formulaires-pdf', title: 'Formulaires PDF', description: 'Remplissez les champs existants ou créez vos propres champs interactifs.', category: 'PDF', icon: FormInput, accent: 'indigo' },
  ...
  { to: '/compresser-video', title: 'Compresser une vidéo', description: 'Réduisez le poids d’une vidéo avant de la partager ou de l’archiver.', category: 'Optimiser', icon: Video, accent: 'violet' },
  { to: '/ocr-pdf', title: 'OCR PDF & images', description: 'Extrayez le texte d’un document pour le copier, le corriger ou le réutiliser.', category: 'Documents', icon: ScanText, accent: 'indigo' },
  { to: '/creer-pdf', title: 'Créer un PDF', description: 'Créez rapidement un document PDF propre avec contenu, logo et pied de page.', category: 'Documents', icon: FileText, accent: 'amber' },
...
const accentClasses: Record<Accent, { icon: string; ring: string; hover: string }> = {
  indigo: { icon: 'bg-indigo-50 text-indigo-600', ring: 'group-hover:border-indigo-200', hover: 'group-hover:text-indigo-600' },
  violet: { icon: 'bg-violet-50 text-violet-600', ring: 'group-hover:border-violet-200', hover: 'group-hover:text-violet-600' },
  cyan: { icon: 'bg-cyan-50 text-cyan-700', ring: 'group-hover:border-cyan-200', hover: 'group-hover:text-cyan-700' },
  emerald: { icon: 'bg-emerald-50 text-emerald-700', ring: 'group-hover:border-emerald-200', hover: 'group-hover:text-emerald-700' },
  amber: { icon: 'bg-amber-50 text-amber-700', ring: 'group-hover:border-amber-200', hover: 'group-hover:text-amber-700' },
};

function ToolCard({ tool }: { tool: ToolItem }) {
  const accent = accentClasses[tool.accent];
  return (
    <Link to={tool.to} className={`group flex min-h-44 flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl ${accent.ring}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <ArrowRight size={17} className={`mt-2 text-slate-300 transition-all group-hover:translate-x-1 ${accent.hover}`} />
      </div>
...
export default function LandingPage() {
  ...
  return (
    <main className="flex-grow bg-slate-50 pt-16 text-slate-950">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-slate-300 md:text-lg md:leading-8">
              Tous les outils essentiels pour modifier, fusionner, signer, compresser et convertir vos PDF et documents — sans compte et sans parcours compliqué.
            </p>
      ...
      <section className="relative z-10 -mt-1 border-b border-slate-200 bg-white px-6 py-8">
        <div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-3">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3.5">
            ...
            <div><p className="text-sm font-bold">Pas de quota quotidien</p><p className="mt-0.5 text-xs text-slate-500">Pas de compteur artificiel à surveiller.</p></div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mb-7 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-[-0.03em] text-slate-950">Commencez par l’essentiel.</h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-slate-500">Quatre actions reviennent le plus souvent. Elles restent accessibles en un clic, sans passer par un tableau de bord compliqué.</p>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Link to="/modifier-pdf" className="group relative min-h-72 overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-600 p-7 text-white shadow-xl shadow-indigo-200/50 transition hover:-translate-y-0.5 hover:shadow-2xl">
              ...
              <div className="relative flex h-full flex-col">
                ...
                <p className="mt-3 max-w-lg text-sm leading-6 text-indigo-100">Texte, images, signatures, annotations, dessin et organisation des pages dans un même espace de travail.</p>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {featuredTools.slice(1).map(tool => (
                <Link key={tool.to} to={tool.to} className="group flex min-h-32 flex-col rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-lg">
                  ...
                  <h3 className="mt-4 font-bold text-slate-950">{tool.title}</h3>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-[-0.03em]">Trouvez directement l’action dont vous avez besoin.</h2>
            </div>
            <div className="relative w-full lg:max-w-md">
              ...
              <input
                ...
                placeholder="Rechercher : signer, OCR, compresser…"
                aria-label="Rechercher un outil"
                className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100"
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-2" role="group" aria-label="Filtrer les outils par catégorie">
            {categories.map(item => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-bold transition ${category === item ? 'bg-slate-950 text-white shadow-md' : 'border border-slate-200 bg-white text-slate-600 hover:border-indigo-200 hover:text-indigo-600'}`}
              >
                {item}
              </button>
            ))}
          ...
          {filteredTools.length > 0 ? (
            ...
            <div className="mt-7 rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-14 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">Essayez un autre mot-clé ou affichez toutes les catégories.</p>
              <button type="button" onClick={() => { setSearch(''); setCategory('Tous'); }} className="mt-5 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white">Réinitialiser la recherche</button>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-7 ring-1 ring-inset ring-indigo-100 md:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              ...
              <p className="mt-4 max-w-xl text-sm leading-7 text-slate-600">Doxali privilégie des parcours courts : choisir une action, déposer le fichier, prévisualiser, modifier si nécessaire puis récupérer le résultat.</p>
            </div>
