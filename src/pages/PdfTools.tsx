        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
          {tools.map(item => (
            <button
              ...
              className={`text-left rounded-2xl border p-5 transition-all ${tool === item.id ? 'bg-gray-900 border-gray-900 text-white shadow-lg' : 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}
            >
          ...
          <div className="p-6 md:p-8">
            <label className="block border-2 border-dashed border-gray-200 hover:border-gray-400 rounded-2xl p-10 text-center cursor-pointer transition-colors">
              <input
                ...
                className="hidden"
                multiple={multiple}
                accept={acceptsImages ? '.png,.jpg,.jpeg,.webp,.ico,image/*' : '.pdf,application/pdf'}
